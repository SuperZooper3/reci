      ...
      <div className="flex justify-between">
        <div>
          ...
          <div className="recipe-date text-sm text-gray-500 mb-2">
              Created by {recipe.display_name != null ? <Link to={`/recipe/${recipe.author_id}`} className="hover:underline">{recipe.display_name}</Link> : "unknown user"} on {new Date(recipe.created_at).toLocaleDateString()}
          </div>
        </div>
